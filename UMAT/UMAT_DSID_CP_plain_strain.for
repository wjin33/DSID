...
C
	...
	2 EPSE(3,3),EPSEL(3,3),DEPSE(3,3),DEPSID(3,3),
     ...
     4 OMEGA_V(4),EPSEL_V(4),EPSE_V(4),EPSID_V(4),
	5 YD1_V(4),ENERGY(7),OMEGA_O(3,3),EPSEL_O(3,3),EPSE_O(3,3),
...
C
      ...
      OMEGA_12 = PROPS(12)
C=============================================================== 
C      DEFAULT NTENS = 4 FOR PLANE STRAIN WITH
C      NDI = 3 NUMBER OF DIRECT STRESS COMPONENS
C      NSHR= 1 NUMBER OF ENGINEERING SHEAR STRESS COMPONENTS
C ---  Get state variables from STATEV(vector) 
      ...
      END DO
	ENERGY(5) = STATEV(1+6*NTENS)
	ENERGY(6) = STATEV(2+6*NTENS)
	ENERGY(7) = STATEV(3+6*NTENS)
      FD = STATEV(4+6*NTENS)
C
...
C      WRITE(7,*),'NSHR=', NSHR
        ...
        OMEGA_O(1,2) = OMEGA_12
	  OMEGA_O(2,1) = OMEGA_12
      END IF
...
C
          ...
          IF (ABS(FD2).GT.FTOL2) THEN
            ...
            INC = INC+1
          END IF
C          WRITE(7,*),'SIGMA_i=',SIGMA
C          WRITE(7,*),'OMEGA_i=',OMEGA
C          WRITE(7,*),'EPSID_i=',EPSID
        END DO
C
        CALL DMAT(AMATDOM,SOM,OMEGA)
        CALL Aijkl_Bkl(SOM,SIGMA,EPSE)
...
C
      ...
      END DO
	  STATEV(1+6*NTENS) = ENERGY(5)+STATEV(1+6*NTENS)
	  STATEV(2+6*NTENS) = ENERGY(6)+STATEV(2+6*NTENS)
	  STATEV(3+6*NTENS) = ENERGY(7)+STATEV(3+6*NTENS)
      CALL FDDP(FD2,SIGMA,OMEGA,YD1)
      IF (FD2.GT.1.E-2) THEN
        STATEV(4+6*NTENS) = 1.E-2
      ELSE
        STATEV(4+6*NTENS) = FD2
      END IF
C
